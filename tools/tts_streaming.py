...
def resolve_streaming_provider(
    ...
    from tools.tts_tool import _get_named_provider_config, _is_command_provider_config
...
#
# The provider reads text from stdin and writes MP3/WAV bytes to the output
# file path (the same contract as the sync command path). We then decode that
# file to PCM and stream the chunks. This avoids requiring the command script
# to support true streaming — any working command provider gets streaming as
# a free upgrade by adding one config line.
# ---------------------------------------------------------------------------
